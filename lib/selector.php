...
/*
 Based on the Selector library by TJ Holowaychuk from https://github.com/tj/php-selector
 * Modified to avoid function and class name collision for inclusion in the WordPress.org repository.
 * Fixed some PHP7 notices
 * Added select_element wrapper method
...
class l2p_SelectorDOM {
	...
	*/	
	...
	public static function select_elements($selector, $html, $as_array = true) {
		$dom = new l2p_SelectorDOM($html);
		return $dom->select($selector, $as_array);
	}
	
	/**
	 * Grab the first element found for a specific selector
	 */
	public static function select_element($selector, $html, $as_array = true) {
		$elements = l2p_SelectorDOM::select_elements($selector, $html, $as_array);
		
		if(is_array($elements) && !empty($elements[0]))
			return $elements[0];
		else
			return $elements;
	}
